...
public static class HelperFunctions
{
    ...
    public static void Hide(this GameObject parent,float Time=0.25f)
    {
       ...
       g.DOFade(0, Time).OnComplete(() => { parent.SetActive(false);
    ...
    public static void Show(this GameObject parent)
    {
        ...
        g.DOFade(1, 0.25f).SetDelay(0.25f).OnStart(() => {  g.alpha = 0;
            parent.SetActive(true);});
    }
    public static TypeWriter Instance;
    public static void Type(this TextMeshProUGUI obj,string txt,float Time=0,string end="")
    {
        if (Instance == null)
        {
        ...
        Instance.Type(obj, txt,Time,end);
    }
    ...
    {
        public void Type(TextMeshProUGUI Text, string t,float tt,string end)
        {
            StopAllCoroutines();
            
            StartCoroutine(PlayText(Text, t,tt,end));
        }
        public IEnumerator PlayText(TextMeshProUGUI txt, string story,float TimeToComp,string end)
        {
            yield return new WaitForSeconds(TimeToComp);
            foreach (char c in story)
            {
                txt.text += c;
                yield return new WaitForEndOfFrame();
              //  yield return new WaitForSeconds((TimeToComp/100));
            }
